"""
FastAPI Routes für Price Movers API (FIXED for Body parameters)
...
async def analyze_price_movers(
    request: AnalysisRequest = Body(...),  # ← FIXED: Explizit als Body
...
async def quick_analysis(
    request: QuickAnalysisRequest = Body(...),  # ← FIXED: Explizit als Body!
    analyzer: PriceMoverAnalyzer = Depends(get_analyzer),
    request_id: str = Depends(log_request)
) -> AnalysisResponse:
    """
...
async def historical_analysis(
    request: HistoricalAnalysisRequest = Body(...),  # ← FIXED: Explizit als Body
    analyzer: PriceMoverAnalyzer = Depends(get_analyzer),
    request_id: str = Depends(log_request)
) -> HistoricalAnalysisResponse:
    """
...
async def compare_exchanges(
    request: CompareExchangesRequest = Body(...),  # ← FIXED: Explizit als Body
...
async def health_check(
    ...
    ### Returns:
    - Gesamt-Status (healthy/degraded)
