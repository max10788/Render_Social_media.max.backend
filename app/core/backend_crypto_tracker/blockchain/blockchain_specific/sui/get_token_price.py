                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            if data.get('result'):
                                logger.info(f"Found token on Sui chain: {token_address}")
                            else:
                                logger.warning(f"Token {token_address} not found on Sui chain")
                        else:
                            logger.warning(f"Sui RPC returned status {response.status}")
            except Exception as e:
                logger.warning(f"Error using Sui RPC: {e}")
        
        # CoinGecko API - Token Price by Contract Address (Sui)
        url = "https://api.coingecko.com/api/v3/simple/token_price/sui"
        params = {
